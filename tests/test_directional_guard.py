...
def controller(plan=FORWARD_PLAN, **safety):
    ...
    if safety:
        job = job.__class__(
            **{**job.__dict__, "safety": job.safety.__class__(**{**job.safety.__dict__, **safety})}
        )
...
def test_reversing_is_judged_on_what_is_behind_not_in_front():
    ...
    blocked = drive(
        c, RangeField(forward=2.0, rear=0.10, left=2.0, right=2.0, closest=0.10, directional=True)
    )
...
def test_reversing_is_not_stopped_by_something_ahead_of_it():
    ...
    assert (
        drive(
            c,
            RangeField(forward=0.15, rear=2.0, left=2.0, right=2.0, closest=0.15, directional=True),
        ).linear_x
        != 0.0
    )
...
def test_turning_in_place_watches_every_side():
    ...
    guarded = c._obstacle_guard(
        RangeField(forward=2.0, rear=0.12, left=2.0, right=2.0, closest=0.12, directional=True),
        0.3,
...
def test_nothing_may_be_nearer_than_the_emergency_distance_in_any_direction():
    ...
    stopped = drive(
        c, RangeField(forward=2.0, left=0.04, right=2.0, rear=2.0, closest=0.04, directional=True)
    )
...
def test_the_emergency_floor_applies_while_reversing_too():
    ...
    stopped = drive(
        c, RangeField(forward=0.05, rear=2.0, left=2.0, right=2.0, closest=0.05, directional=True)
    )
    assert stopped.linear_x == 0.0
    assert "emergency" in c.events[-1].detail
